...
class Game:
	...
	def someone_won(self, symbolOfCurPlayer, Cells, countMaxWinSymb):
		for i in range(self.Y):#горизонталь
			nInRow = 0
			for j in range(self.X):
				if(Cells[i][j] != symbolOfCurPlayer):
					nInRow = 0
				else:
					nInRow+=1
		...
		for i in range(self.X):#вертикаль
			nInRow = 0
			for j in range(self.Y):
				if(Cells[j][i] != symbolOfCurPlayer):
					nInRow = 0
				else:
					nInRow+=1
					if nInRow >= countMaxWinSymb:
		...
		for j in range(self.Y):#диагональ вверх
			nInRow = 0
			i = 0
			while(self.isInField(i, i+j)):
				if(Cells[i+j][i] != symbolOfCurPlayer):
						nInRow = 0
				else:
					nInRow+=1
					if nInRow >= countMaxWinSymb:
						return True
				i+=1
		for j in range(1, self.X):#диагональ вниз
			nInRow = 0
			i = 0
			while(self.isInField(i+j, i)):
				if(Cells[i][i+j] != symbolOfCurPlayer):
						nInRow = 0
				else:
					nInRow+=1
					if nInRow >= countMaxWinSymb:
						return True
				i+=1
		for j in range(self.Y):#обратная диагональ вверх
			nInRow = 0
			i = 0
			while(self.isInField(-i-1, i+j)):
				if(Cells[i+j][-i-1] != symbolOfCurPlayer):
						nInRow = 0
				else:
					nInRow+=1
					if nInRow >= countMaxWinSymb:
						return True
				i+=1
		for j in range(0, self.X-1):#обратная диагональ вниз
			nInRow = 0
			i = 0
	...
	def run(self):
		while not self.someone_won(self.__t, self.rebCells(self.__f._Field__cells), self.__l):
			self.__f.show()
			self.turn()
	...
	def rebCells(self, arr):
		newArr = []
		for i in arr:
			artmp = []
			for j in i:
				artmp.append(j.__state)
			newArr.append(artmp)
		return newArr
